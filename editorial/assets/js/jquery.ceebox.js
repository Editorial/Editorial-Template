...
$.fn.ceebox.defaults = {
	...
	margin: 0, //minimum margin between ceebox inside content and browser frame (this does not count the padding and border; I know it's odd. I'll likely change how it works at some point)
	//misc settings
...
$.fn.ceebox.popup = function(content,opts) {
	...
	if ($(content).is("a,area,input") && (opts.type == "html" || opts.type == "image" || opts.type == "video")) { //
		...
		if($.browser.msie) opts.width = 960;
	...
	$("#cee_box").animate(animOpts,opts.animSpeed,opts.easing,function(){
			...
			if (opts.modal===true) {
				...
				// 7a. add closebtn
				$("<a href='#' id='cee_closeBtn' class='cee_close'>Nazaj na članek</a>").prependTo("#controls");
...
var BoxAttr = function(cblink,o) {
	...
	this.titlebox = (o.titles) ? "<p id='cee_title' style='width:"+ w +"px;margin:0 auto;'>"+this.title+"</p><div id='controls'><div id='cee_logo'></div></div>" : "";
	this.width = w;
	this.height = h;
	this.rel = rel;
...
function pageSize(margin){
	...
	this.height = (window.innerHeight || self.innerHeight || (de&&de.clientHeight) || document.body.clientHeight) - margin - 60;
	return this;
}
...
function addGallery(g,family,opts){ // adds gallery next/prev functionality
	...
	function navLink(btn,id) {
		...
		var btnN = btn;
		(btnN == "prev") ? btnN = 'Predhodna' : btnN = 'Sledeča';

		var style = function(y) {return $.extend({zIndex:105},s[0]);};
		...
		$("<a href='#'></a>").text(btnN).attr({id:"cee_" + btn}).css(style(off)).hover(
			...
			}).wrapInner('<span />').appendTo("#controls");
	}
	
	// add prev/next buttons
	if (g.prevId >= 0) {navLink("prev",g.prevId);}
	if (g.nextId) {navLink("next",g.nextId);}
	//$("#controls").append("<div id='cee_count'>" + (g.gNum+1) +" / "+ g.gLen + "</div>");

	if ($('a#cee_prev').length >= 1 ) $("a#cee_prev").after("<div id='cee_count'>" + (g.gNum+1) +" / "+ g.gLen + "</div>");
	else if ($('a#cee_next').length >= 1 ) $("a#cee_next").before("<div id='cee_count'>" + (g.gNum+1) +" / "+ g.gLen + "</div>");
	else $("#controls").append("<div id='cee_count'>" + (g.gNum+1) +" / "+ g.gLen + "</div>");
