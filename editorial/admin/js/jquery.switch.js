...
// MODIFIED!!!!
// had to remove change bind on the switch, as it was causing not to fire the event
...
;(function($, doc) {
  ...
  <div class="gallery-switch">                              \n\
    ...
    <div class="text">{{galleryState}}</div>                \n\
  </div>';
  ...
  var switchify = {
    ...
    build: function($select, options) {
      ...
      var values = {
        ...
        galleryState: gallerySwVal1
      };

      // assign the <select>'s val as a class on the switch
      var $switch = $(template.replace('{{on}}', text.on).replace('{{off}}', text.off).replace('{{galleryState}}', text.galleryState));
